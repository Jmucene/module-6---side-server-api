...
// user search of Today's and the weekly weather data
...
// generates the user's previous searches, returns empty if no data
...
//local storage for the weather data the user searched
function getCurrentWeatherConditions(citySearched) {
  ...
  // API call for current weather if none in local storage
  $.ajax({
    url: queryURL,
    method: "GET"
...
// Gets current weather data from storage or the API call
...
// finds the UV Index for city in local storage, or API to get the data
...
// Takes storage or API data and displays on page
function populateForecast(results) {
  $("#forecast").empty();
  let list = results.list;
  let daysForecasted = 0;  // Checks for only 5 days showing
...
// Formating openweathermaps API date format to "MM/DD/YYY"
